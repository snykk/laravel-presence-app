...
namespace App\Vendors\I18n\Eloquent\Extensions;

use App\Vendors\I18n\Eloquent\Observer;
use App\Vendors\I18n\Eloquent\TranslationModel;
...
trait TranslatableWithSlug
{
     ...
     *
     * @var \App\Vendors\I18n\Eloquent\TranslationModel|null
     */
     ...
     * @return void
     */
    public static function bootTranslatableWithSlug(): void
    {
     ...
     *
     * @return \App\Vendors\I18n\Eloquent\TranslationModel
     */
    ...
    protected function initialize(): void
    {
        ...
        if (!isset($this->fallbackTranslation)) {
            ...
            $fallbackTranslation = $this->translations->where('locale', $locale)->first();

            $this->fallbackTranslation = null;

            if ($fallbackTranslation instanceof TranslationModel) {
    ...
    public function translate($key = null): self
    {
        ...
        $usedTranslation = $this->translations->where('locale', $key)->first();

        $this->translation = null;

        if ($usedTranslation instanceof TranslationModel) {
            $this->translation = $usedTranslation;
        }
    ...
    public function resolveRouteBinding($value, $field = null)
    {
        if ($this->getRouteKeyName() == 'slug') {
            $routedLocale = \I18n::routedLocale();
            $locale = $routedLocale ? $routedLocale->language : \App::getLocale();
            $attributes = $this->getJoinAttributes();
            ...
            ->where($this->getTranslationTable().'.locale', $locale)
            ->where('slug', $value)
            ->first();
        }
